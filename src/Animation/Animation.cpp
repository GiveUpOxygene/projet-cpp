...
void Animation::moveUp(float speed, int minY){
    sf::FloatRect entityRect = this->getRect();

    if(entityRect.top - speed >= minY)
        this->setPos(entityRect.left, entityRect.top - speed);
}
...
void Animation::moveDown(float speed, int maxY){
    ...
    if(entityRect.top + entityRect.height + speed <= maxY)
        this->setPos(entityRect.left, entityRect.top + speed);
}
...
void Animation::moveLeft(float speed, int minX){
    ...
    if(entityRect.left - speed >= minX)
        this->setPos(entityRect.left - speed, entityRect.top);
}
...
void Animation::moveRight(float speed, int maxX){
    ...
    if(entityRect.left + entityRect.width + speed <= maxX)
        this->setPos(entityRect.left + speed, entityRect.top);
}
