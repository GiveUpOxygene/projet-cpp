...
void filloss(std::ostream *stream,
             const T (&in)[N]) { // NOLINT(*-avoid-c-arrays)
...
#define DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id, anon, ...)             \
  DOCTEST_GLOBAL_NO_WARNINGS(                                                  \
      DOCTEST_CAT(anon, DUMMY), /* NOLINT(cert-err58-cpp,                      \
...
#define DOCTEST_EXCEPTION_EMPTY_FUNC                                           \
  [] {                                                                         \
    static_assert(false,                                                       \
                  ...
                  "Use DOCTEST_CONFIG_NO_EXCEPTIONS_BUT_WITH_ALL_ASSERTS if "  \
                  "you want to compile with exceptions disabled.");            \
    return false;                                                              \
  }()
...
public:
  ...
  T operator=(T desired)
      DOCTEST_NOEXCEPT { // lgtm [cpp/assignment-does-not-return-this]
    store(desired);
    return desired;
  }
...
void addAssert(assertType::Enum at) {
  if ((at & assertType::is_warn) == 0) //! OCLINT bitwise operator in
                                       //! conditional
...
void fulltext_log_assert_to_stream(std::ostream &s, const AssertData &rb) {
  ...
  } else if (rb.m_at & assertType::is_throws_with) { //! OCLINT bitwise operator
                                                     //! in conditional
    s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr << ", \""
